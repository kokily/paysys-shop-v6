...
export class UsersService {
   ...
   * - 전체 사용자 목록 조회
   * - 필터링 지원: username
   * - 페이진네이션 (Cursor 기반, 20개씩)
   * - 생성일 역순 정렬
   * - 비밀번호 제외 반환
   * @param listUsersDto 조회 조건
   * @returns 사용자 목록 (비밀번호 제외)
   */
  async findAll(listUsersDto: ListUsersDto): Promise<Partial<User>[]> {
    const { username, cursor } = listUsersDto;
    ...
    if (cursor) {
      ...
      if (!cursorUser) {
        throw new NotFoundException('커서 사용자를 찾을 수 없습니다.');
      }

      query.andWhere(
        '(users.created_at < :date OR (users.created_at = :date AND users.id < :id))', {
  ...
  /**
   * 관리자 권한 설정
   * - 특정 사용자의 관리자 권한을 설정/해제
   * @param id 사용자 ID
   * @param isAdmin 관리자 권한 여부
   * @returns 설정 성공 메시지
   */
  async setAdmin(id: string, isAdmin: boolean): Promise<{ message: string }> {
    const user = await this.userRepository.findOne({ where: { id } });
  ...
  async changePassword(userId: string, newPassword: string): Promise<{ message: string }> {
    ...
    if (!user) {
      throw new NotFoundException('접속된 사용자가 없습니다.');
    }
